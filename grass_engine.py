...
class GrassEngine:
    def __init__(self):
        self.grass_binary = shutil.which('grass78') or \
        ...
        if self.grass_binary is None:
            logger.warning(
                "Could not find a GRASS 7 executable. GRASS scripts will not work.")
        else:
            logger.info("Initializing GRASS engine using {}".format(
                self.grass_binary))
...
class GrassContext:
    def __init__(self, grass_binary, tmpdir=None, script_opts={}, location=None, auto_cleanup=True, python_path=None):
        ...
        if tmpdir is None:
            tmpdir = os.path.basename(tempfile.mkdtemp(
    ...
    def get_cwd(self):
        return os.path.join(MEDIA_TMP, self.tmpdir)
    ...
    def execute(self, script):
        ...
        if self.python_path:
            ...
            env["PYTHONPATH"] = "%s%s%s" % (
                self.python_path, sep, env.get("PYTHONPATH", ""))

        # Execute it
        logger.info("Executing grass script from {}: {} -c {} location --exec python {} {}".format(
            self.get_cwd(), self.grass_binary, self.location, script, " ".join(params)))
        command = [self.grass_binary, '-c', self.location,
                   'location', '--exec', 'python', script] + params
        if platform.system() == "Windows":
            # communicate() hangs on Windows so we use check_output instead
            try:
                out = subprocess.check_output(
                    command, cwd=self.get_cwd(), env=env).decode('utf-8').strip()
                success = True
            except:
                success = False
                err = out
            ...
            logger.info("Command output: {}".format(str(out)))
            logger.info("Command err: {}".format(str(err)))
        ...
        if success:
            ...
            raise GrassEngineException(
                "Could not execute GRASS script {} from {}: {}".format(script, self.get_cwd(), err))
